...
Spectator.describe Store::Keyer do
  ...
  describe ".term_to_iids" do
    it "keys term to iids" do
      ...
      expect(result1).to eq(Bytes[1, 50, 220, 166, 65, 131, 225, 5, 46])
      expect(result2).to eq(Bytes[1, 50, 220, 166, 65, 204, 96, 136, 213])
    end
  end
  ...
  describe ".oid_to_iid" do
    it "keys oid to iid" do
      ...
      expect(result).to eq(Bytes[2, 171, 194, 213, 57, 31, 156, 118, 213])
    end
  end
  ...
  describe ".iid_to_oid" do
    it "keys iid to oid" do
      ...
      expect(result).to eq(Bytes[3, 105, 12, 54, 147, 230, 11, 157, 0])
    end
  end
