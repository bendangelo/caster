module Pipe
  ...
  struct PushParams
    ...
    property collection : String, bucket : String, object : String, lang : String?, attrs : Array(UInt32)?, text : String, keywords : String?, headers : String?
  end
    ...
    def self.dispatch_push(input : String) : CommandResult
      ...
      if collection && bucket && object && text
        ...
        if text.blank?
          ...
          token = Lexer::TokenBuilder.from(mode, text, hinted_lang, Caster.settings.search.term_index_limit, params.keywords, params.headers)
